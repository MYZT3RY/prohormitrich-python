...
def cmdMyStats(update: Update, context: CallbackContext):
    ...
    context.bot.send_message(chat_id=tgChatId, text="<b>Личная статистика в чате</b>\n\nДата регистрации в чате: <b>{0}</b>\n\nСообщений: <b>{1}</b>\nГолосовых сообщений: <b>{2}</b>\nВидео сообщений: <b>{3}</b>\nВидеозаписи: <b>{4}</b>\nАудиозаписи: <b>{5}</b>\nФотографий: <b>{6}</b>\nДокументов: <b>{7}</b>\nСтикеров: <b>{8}</b>\n\nСреднее количество сообщений в день: <b>{9}</b>".format(row["dateofregister"],row["messages"],row["voices"],row["videovoices"],row["videos"],row["audios"],row["photos"],row["documents"],row["stickers"],messagesPerDay), parse_mode=PARSEMODE_HTML)