...
def cmdStats(update: Update, context: CallbackContext):
    ...
    context.bot.send_message(chat_id=tgChatId, text="<b>Статистика чата</b>\n\nДата регистрации чата: <b>{0}</b>\nКоличество участников чата: <b>{1}</b> (<b>{2}</b> зарегистрированных)\n\nСообщений: <b>{3}</b>\nГолосовых сообщений: <b>{4}</b>\nВидео сообщений: <b>{5}</b>\nВидеозаписи: <b>{6}</b>\nАудиозаписи: <b>{7}</b>\nФотографий: <b>{8}</b>\nДокументов: <b>{9}</b>\nСтикеров: <b>{10}</b>\n\nСреднее количество сообщений в день: <b>{11:.2f}</b>".format(row["dateofregister"],totalUsers,registeredUsers,row["messages"],row["voices"],row["videovoices"],row["videos"],row["audios"],row["photos"],row["documents"],row["stickers"],messagesPerDay), parse_mode=PARSEMODE_HTML, disable_notification=True)