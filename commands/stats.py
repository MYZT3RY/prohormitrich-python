...
def cmdStats(update: Update, context: CallbackContext):
    ...
    context.bot.send_message(chat_id=tgChatId, text="<b>Статистика чата - {0}</b>\n\nДата регистрации чата: <b>{1}</b>\nКоличество участников чата: <b>{2}</b> (<b>{3}</b> зарегистрированных)\n\nСообщений: <b>{4}</b>\nГолосовых сообщений: <b>{5}</b>\nВидео сообщений: <b>{6}</b>\nВидеозаписи: <b>{7}</b>\nАудиозаписи: <b>{8}</b>\nФотографий: <b>{9}</b>\nДокументов: <b>{10}</b>\nСтикеров: <b>{11}</b>\n\nСреднее количество сообщений в день: <b>{12:.2f}</b>".format(tgChatName,row["dateOfRegister"],totalUsers,row["registeredusers"],row["messages"],row["voices"],row["videovoices"],row["videos"],row["audios"],row["photos"],row["documents"],row["stickers"],messagesPerDay), parse_mode=PARSEMODE_HTML, disable_notification=True)